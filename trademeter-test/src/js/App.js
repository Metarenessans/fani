...
export default class App extends React.Component {
   ...
   */
  getMinDailyIncome(custom,real=false) {
    const {
      mode,
      depoEnd,
      withdrawalInterval,
	...
	console.log('RD', this.state.realData);
	
	const factDays = [];
	if (real)  factDays = this.state.realData;
                  ...
                  <div className="controls-card4">
                    ...
                    <label className="input-group">
                      ...
                      <NumericInput
                        ...
                        disabled={this.state.saved}
                        className="input-group__input"
                        defaultValue={this.state.withdrawalShow[this.state.mode]}
                        round="true"
                        unsigned="true"
                        format={formatNumber}
                        max={
                          this.state.mode == 0
                            ? this.state.depoStart[this.state.mode] * 0.15 * this.state.withdrawalInterval //e
                            : this.state.depoStart[this.state.mode] * (this.state.incomePersantageCustom / 100 * this.state.withdrawalInterval) //e
                        }
                        onBlur={val => {
                          //this.setWithdrawal(val); //e
						  this.setWithdrawal(val * (1 + comissionPercent) + comissionFixPart); //e
