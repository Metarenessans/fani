...
class App extends React.Component {
  ...
  fetchInitialData() {
    ...
    this.fetchTools().then(() => this.fetchCompanyQuotes());
  }
  ...
  fetchSaves() {
    return new Promise((resolve, reject) => {
      fetch("getMtsSnapshots")
        .then((res) => {
          const savesSorted = res.data.sort(
    ...
    return new Promise((resolve) => {
      for (let request of ["getFutures", "getTrademeterInfo"]) {
        fetch(request)
          ...
          .then(resolve)
          .catch((error) =>
            this.showAlert(`Не удалось получить инстурменты! ${error}`)
          );
      }
    });
  }
  ...
  fetchInvestorInfo() {
    fetch("getInvestorInfo")
      .then((response) => {
        ...
        return new Promise((resolve) => {
          this.setState({ investorInfo: { status, skill } }, () =>
            resolve(deposit)
          );
        });
      })
  ...
  updatePriceRange(tool) {
    return new Promise((resolve) => {
      ...
      this.setState({ priceRange: [currentPrice, currentPrice] }, () =>
        resolve()
      );
    });
  }
  ...
  extractSave(save) {
    ...
    try {
      ...
      state.customTools = state.customTools.map((tool) =>
        Tools.create(tool, { investorInfo: this.state.investorInfo })
      );
      ...
      fetch("addMtsSnapshot", "POST", data)
        .then((res) => {
          ...
          if (id) {
            ...
            reject(
              `Произошла незвестная ошибка! Пожалуйста, повторите действие позже еще раз`
            );
          }
        })
        .catch((err) => reject(err));
  ...
  delete(id = 0) {
    ...
    return new Promise((resolve, reject) => {
      fetch("deleteMtsSnapshot", "POST", { id })
        .then(() => {
          let { id, saves, saved, changed, currentSaveIndex } = this.state;

          saves.splice(currentSaveIndex - 1, 1);

          currentSaveIndex = Math.min(
            Math.max(currentSaveIndex, 1),
            saves.length
          );
          ...
          if (saves.length > 0) {
            ...
            this.reset().catch((err) => this.showAlert(err));
  ...
  render() {
    ...
    const priceRangeSorted = [...priceRange]
      .sort((l, r) => l - r)
      .map((sorted) => round(sorted, 2));

    const planIncome = round(priceRangeSorted[1] - priceRangeSorted[0], 2);
    const contracts = Math.floor(
      (depo * (Math.abs(percentage) / 100)) / currentTool.guarantee
    );
    ...
    let income =
      (((contracts || 1) * planIncome) / currentTool.priceStep) *
    ...
    const GetPossibleRisk = () => {
      ...
      let stopSteps =
        ((depo * risk) / 100 / currentTool.stepPrice / (contracts || 1)) *
    ...
    return (
      <div className="page">
                ...
                {(() => {
                  ...
                  return (
                    (dev || saves.length > 0) && (
                      <label
                        hidden
                        className="labeled-select main-top__select stack-exception"
                      >
              ...
              <div className="main-content__wrap">
                    ...
                    <Stack className="main-content__left">
                      <label>
                        ...
                        <Select
                          ...
                          onChange={(currentToolIndex) => {
                            const tools = this.getTools();
                            const currentToolCode =
                              tools[currentToolIndex].code;
                            this.setStateAsync({ currentToolCode })
                              .then(() =>
                                this.updatePriceRange(
                                  this.getToolByCode(currentToolCode)
                                )
                              )
                      ...
                      <div className="mts-slider1">
                        <span className="mts-slider1-middle">
                          <b>Текущая цена</b>
                          <br />({formatNumber(price)})
                        </span>
                        <span className="mts-slider1-top">
                          <b>
                            {formatNumber(round(max - scaleOffset, fraction))}
                          </b>
                          &nbsp; (+
                          {round(
                            (percent / currentTool.currentPrice) * 100 +
                              movePercantage,
                            2
                          )}
                          %)
                        ...
                        <span className="mts-slider1-bottom">
                          <b>
                            {formatNumber(round(min + scaleOffset, fraction))}
                          </b>
                          &nbsp; (-
                          {round(
                            (percent / currentTool.currentPrice) * 100 +
                        ...
                        <CustomSlider
                          ...
                          tipFormatter={(val) =>
                            formatNumber(val.toFixed(fraction))
                          }
                          onChange={(priceRange) => {
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            const sliderStepPercent = 0.5;
                            const scaleStep = round(price * 0.005, 2);
                            ...
                            if (min + updatedScaleOffset != 0) {
                              this.setState({
                                ...
                                movePercantage:
                                  movePercantage + sliderStepPercent,
                                days: 0,
                              });
                              updateChartScaleMinMax(
                                min + updatedScaleOffset,
                                max - updatedScaleOffset
                              );
                            }
                          }}
                        ...
                        <Button
                          ...
                          disabled={(() => {
                            let { scaleOffset } = this.state;
                            let scaleStep = round(price * 0.005, 2);
                            const updatedScaleOffset = scaleOffset + scaleStep;
                            return (
                              Math.abs(
                                min +
                          ...
                          onClick={() => {
                            ...
                            Math.abs(
                              min +
                                updatedScaleOffset +
                                scaleStep -
                                (max - updatedScaleOffset + scaleStep)
                            );
                            if (
                              min + updatedScaleOffset <
                              max - updatedScaleOffset
                        ...
                        <div className="main-content-stats__wrap">
                          <div className="main-content-stats__row">
                            ...
                            <NumericInput
                              ...
                              unsigned="true"
                              format={(number) =>
                                formatNumber(round(number, fraction))
                              }
                              round={false}
                              defaultValue={
                                (isLong ? priceRange[0] : priceRange[1]) || 0
                              }
                              onBlur={(value) => {
                                const callback = () => {
                          ...
                          <div className="main-content-stats__row">
                            ...
                            <NumericInput
                              ...
                              round={false}
                              defaultValue={
                                (isLong ? priceRange[1] : priceRange[0]) || 0
                              }
                              onBlur={(value) => {
                                const callback = () => {
                                  updateChartMinMax(
                          ...
                          <div className="main-content-stats__row">
                            ...
                            <span className="main-content-stats__val">
                              {formatNumber(
                                round(
                                  Math.abs(priceRange[0] - priceRange[1]),
                                  fraction
                                )
                              )}
                            </span>
                ...
                <Stack className="main-content__right">
                  ...
                  {(() => {
                    return (
                      <div
                        className={["mts-slider2"]
                          .concat(percentage >= 0 ? "right" : "")
                          .join(" ")
                          .trim()}
                      >
                        ...
                        <CustomSlider
                          ...
                          onChange={(range = []) => {
                            ...
                            investorInfo.type =
                              percentage >= 0 ? "LONG" : "SHORT";
                            tools = tools.map((tool) =>
                              tool.update(investorInfo)
                            );
                        ...
                        <span aria-hidden="true" className="mts-slider2-short">
                          Short
                        </span>
                        <span aria-hidden="true" className="mts-slider2-long">
                          Long
                        </span>
                      </div>
                    );
        ...
        <Dialog id="settings-generator" pure={true}>
          <SettingsGenerator
