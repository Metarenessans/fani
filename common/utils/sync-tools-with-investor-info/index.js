 ...
 * @returns {Promise}
 */
export default function syncToolsWithInvestorInfo(investorInfo) {
  return new Promise((resolve, reject) => {
    ...
    const updatedTools = tools.map(tool => tool.update(investorInfo));
    ...
    if (!updatedTools.every(tool => {
      ...
      if (typeof guarantee == "object") {
        ...
        if (investorInfo.status && investorInfo.type) {
      ...
      if (comp1 != comp2) {
        // console.error(comp1, comp2, tool, tool.ref);
      }
